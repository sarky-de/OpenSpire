...
export class RefillEnergySystem extends TriggerSystem {
    ...
    public Run(payload?: any): void {
        ...
        const energyComponents = ECS.getInstance().getComponents<EnergyComponent>(EnergyComponent);

        energyComponents.some(component => {
            if (component.entityId == playerEntityId) {
                component.energy = component.maxEnergy;
                Logger.info(`Refilled energy for player ${playerEntityId} to ${component.energy}`);
                return true;
            }
        });
    }
